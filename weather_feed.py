...
WEATHER_CITIES: dict[str, dict] = {
    ...
    # v10.7: città aggiunte per coprire mercati Polymarket mancanti
    "sao paulo": {
        "lat": -23.626,
        "lon": -46.655,
        "keywords": ["sao paulo", "são paulo", "guarulhos"],
        "station": "Guarulhos Intl",
        "wethr_code": "SBGR",
        "nws_grid": None,
        "unit": "C",
    },
    ...
    def _fetch_ensemble(self, city: str, info: dict) -> dict[str, SourceForecast]:
        try:
            ...
            for i, date in enumerate(dates):
                ...
                denom = len(members) - 1 if len(members) > 1 else 1
                variance = sum((t - mean_temp) ** 2 for t in members) / denom
        ...
        try:
            ...
            if isinstance(forecasts_data, list):
                for entry in forecasts_data:
                    ...
                    denom = len(model_temps) - 1 if len(model_temps) > 1 else 1
                    var = sum((t - mean_t) ** 2 for t in model_temps) / denom
                    std = max(var ** 0.5, 0.3)
